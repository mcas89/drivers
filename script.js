...
let nomeMotorista = ''; 
let carregarCorridasAtiva = true; // Variável de controle
...
function loginMotorista() {
  ...
  fetch(`${baseURL}/motoristas/${cpf}.json`)
    ...
    .then(data => {
      ...
      motoristaCreditos = data.creditos || 0;
      bairroPaiMotorista = data.bairroPai;
      nomeMotorista = data.nome; // necessário para o chat
...
function carregarCorridas() {
  if (!carregarCorridasAtiva) return;
  if (!carregarCorridasAtiva) return; // Verifica se a função está ativa
    ...
    .then(data => {
      ...
      for (const id in data) {
        ...
        if (
          ...
          // Mapeia ícones por método de pagamento
          const iconesPagamento = {
            "Débito": "fas fa-credit-card",
            "Crédito": "far fa-credit-card",
            "Pix": "fas fa-qrcode",
          ...
          card.innerHTML = `
            <div style="padding: 8px 10px; background: #f9f9f9; border-radius: 10px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); font-size: 13px; line-height: 1.3;">
              ...
              <div style="display: flex; justify-content: space-between; margin: 6px 0; flex-wrap: wrap; gap: 4px;">
                ...
                <span style="font-size: 16px;"><i class="fas fa-wallet" style="font-size: 20px;"></i> ${creditosNecessarios} créditos</span>
...
function desativarCarregarCorridas() {
  carregarCorridasAtiva = false;
  console.log('Função de carregamento de corridas desativada');

  // Limpa os cards de corridas
  const container = document.getElementById('corridasContainer');
  container.innerHTML = '';

  // Oculta o título "Buscando corridas"
  const titulo = document.getElementById('corridasDisponiveis');
  titulo.style.display = 'none';
}

function ativarCarregarCorridas() {
  carregarCorridasAtiva = true;
  console.log('Função de carregamento de corridas ativada');

  // Exibe o título "Buscando corridas"
  const titulo = document.getElementById('corridasDisponiveis');
  titulo.style.display = 'block';
}

  function toggleCorridas() {
    const toggleButton = document.getElementById('toggleCorridasButton');

    if (carregarCorridasAtiva) {
      desativarCarregarCorridas();
      toggleButton.style.color = 'red';  // Muda a cor para vermelho
    } else {
      ativarCarregarCorridas();
      toggleButton.style.color = 'green';  // Muda a cor para verde
    }
}
...
function fecharCardFinalizada() {
  const cpfMotorista = String(localStorage.getItem('motoristaLogado'));
  const chave = window.ultimaChaveHistorico;
  if (!cpfMotorista || !chave) return;

  const urlHistorico = `${baseURL}/historico_motorista/${cpfMotorista}/${chave}/status.json`;
  ...
  .then(() => {
    ...
    const urlMotorista = `${baseURL}/motoristas/${cpfMotorista}.json`;
    fetch(urlMotorista)
      .then(res => res.json())
      .then(data => {
        if (!data) {
        ...
        // Força número para evitar concatenação de string
        const totalAtual = Number(data.total_viagens) || 0;
        const novoTotal = totalAtual + 1;

        fetch(`${baseURL}/motoristas/${cpfMotorista}/total_viagens.json`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(novoTotal)
        })
        .then(() => {
          console.log(`✅ total_viagens atualizado para ${novoTotal}`);
          monitorarHistoricoMotorista(cpfMotorista);
          location.reload();
        })
        .catch(err => console.error('❌ Erro ao atualizar total_viagens:', err));
      })
      .catch(err => console.error('❌ Erro ao buscar dados do motorista:', err));
  })
  .catch(err => console.error('❌ Erro ao atualizar status da corrida:', err));
}

function mostrarTotalViagens() {
  const cpfMotorista = String(localStorage.getItem('motoristaLogado'));
  if (!cpfMotorista) return;
  ...
  fetch(urlMotorista)
    ...
    .then(data => {
      ...
      const nome = data.nome || 'Nome do Motorista';
      const totalViagens = Number(data.total_viagens) || 0;

      document.getElementById('nomeMotorista').textContent = nome;
      document.getElementById('totalViagens').textContent = `Viagens ${totalViagens}`;
    })
    .catch(err => console.error('Erro ao buscar dados do motorista:', err));
}
...
function abrirCardEditarBairro() {
  const card = document.getElementById('editarBairroCard');
  const select = document.getElementById('novoBairroSelect');

  if (!card || !select) {
    console.error("Card ou select não encontrado");
    return;
  }

  // Limpa e carrega os bairros pai
  select.innerHTML = '';
  for (const bairroPai in bairros) {
    const option = document.createElement('option');
    option.value = bairroPai;
  ...
  if (typeof bairroPaiMotorista !== 'undefined') {
    select.value = bairroPaiMotorista;
  }

  card.style.display = 'block';
}

function fecharCardBairro() {
  const card = document.getElementById('editarBairroCard');
  if (card) {
...
function salvarNovoBairro() {
  const novoBairro = document.getElementById('novoBairroSelect').value;

  fetch(`${baseURL}/motoristas/${motoristaCPF}.json`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ bairroPai: novoBairro })
  })
  .then(res => res.json())
  .then(() => {
    document.getElementById('bairroMotorista').textContent = novoBairro;
    bairroPaiMotorista = novoBairro;
    fecharCardBairro(); // <- ESSENCIAL para fechar o card após salvar
    mostrarAlertaSimples('Bairro atualizado com sucesso!');
  })
  .catch(err => {
    console.error("Erro ao salvar novo bairro:", err);
    alert("Erro ao salvar novo bairro.");
  });
}
